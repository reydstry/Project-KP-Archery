    ...
    window.API.get(`/coach/training-sessions/${sessionId}/bookings${qs.toString() ? `?${qs.toString()}` : ''}`)
        .then(attendance => {
            ...
            participants = bookings.map(b => {
                ...
                // Map status dari server
                let currentStatus = 'present'; // Default hadir
                
                if (b.attendance_status === 'present') {
                    currentStatus = 'present';
                } else if (b.attendance_status === 'absent') {
                    currentStatus = 'absent';
                } else if (!b.has_attendance) {
                    // Belum ada record attendance, default present
                    currentStatus = 'present';
                }
...
function updateSessionDetailsDisplay(sessionId, slotId) {
    ...
    if (slotId && allSlots.length > 0) {
        ...
        if (selectedSlot && selectedSlot.session_time) {
            const st = selectedSlot.session_time;
    ...
    if (filteredParticipants.length === 0) {
        container.innerHTML = `
            <div class="text-center py-12">
                ...
                <p class="text-sm text-slate-500 mt-1">Belum ada booking untuk sesi ini</p>
            </div>
        `;
...
function saveAttendance() {
    ...
    const tasks = participants
        .map(p => {
            ...
            if (!statusChanged) {
                return null; // Tidak ada perubahan, skip
            }
            ...
            if (desiredStatus === 'present') {
                // Member hadir: POST atau PATCH status present
                const payload = { status: 'present', notes: null };
                request = p.has_attendance
                    ? () => window.API.patch(url, payload)
                    : () => window.API.post(url, payload);
                action = 'hadir';
            } else if (desiredStatus === 'absent') {
                // Member tidak hadir: POST atau PATCH status absent
                const payload = { status: 'absent', notes: null };
                request = p.has_attendance
                    ? () => window.API.patch(url, payload)
                    : () => window.API.post(url, payload);
                action = 'tidak_hadir';
            } else {
                // Status lain: tidak perlu action
                return null;
            }

            return { p, request, action };
        })
        .filter(Boolean);
    ...
    if (tasks.length === 0) {
        ...
        showNotification('ℹ️ Tidak ada perubahan untuk disimpan. Ubah status kehadiran terlebih dahulu.', 'info');
        return;
    }
    ...
    (async () => {
        ...
        try {
            for (const task of tasks) {
                await task.request();
                successCount++;
                if (task.action === 'hadir') presentCount++;
                if (task.action === 'tidak_hadir') absentCount++;
            }
            
            // Build detailed success message
            let modalMessage = '';
            const messages = [];
            if (presentCount > 0) messages.push(`${presentCount} member hadir`);
            if (absentCount > 0) messages.push(`${absentCount} member tidak hadir`);
            
            if (messages.length > 0) {
                modalMessage = `Data kehadiran berhasil disimpan: ${messages.join(', ')}.`;
            } else {
                modalMessage = 'Data kehadiran berhasil disimpan ke sistem.';
            }
