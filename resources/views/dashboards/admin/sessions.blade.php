...
<div class="space-y-4">
    <div class="flex flex-col sm:flex-row sm:items-center gap-3">
        <a href="{{ route('admin.sessions.create') }}" class="sm:ml-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-white bg-[#1a307b] hover:bg-[#162a69] text-sm font-semibold transition">
...
async function fetchSessions() {
    ...
    try {
        const data = await window.API.get('/admin/training-sessions');
            ...
            <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <div class="p-4 border-b border-slate-100 flex items-start justify-between gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="/admin/sessions/${session.id}/edit" class="p-2 rounded-lg bg-[#1a307b] text-white hover:bg-[#162a69]" title="Edit sesi">
...
function renderSessionDetails(slots) {
    if (!slots.length) {
    ...
    return slots.map(slot => {
        ...
        const slotBubbleId = `slotDetail-admin-${slot.id}-${(st?.id || '')}`;
        const slotBubbleIconId = `slotDetailIcon-admin-${slot.id}-${(st?.id || '')}`;
...
async function confirmDelete() {
    ...
    try {
        await window.API.delete(`/admin/training-sessions/${sessionToDelete}`);
