...
<div class="space-y-4" x-data="slotCoachPage()" x-init="init()">
    ...
    <div x-show="createModal" x-cloak class="fixed inset-0 z-50">
        <div class="absolute inset-0 bg-black/40" @click="createModal=false"></div>
        <div class="relative min-h-screen flex items-center justify-center p-4">
            <div class="w-full max-w-xl bg-white rounded-xl border border-slate-200 p-5 space-y-4">
                <h3 class="text-lg font-bold text-slate-900">Tambah Slot</h3>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Waktu Slot</label>
                    <select x-model="createForm.session_time_id" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                ...
                <div>
                    ...
                    <input type="number" min="1" max="50" x-model.number="createForm.max_participants" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Coach</label>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto border border-slate-200 rounded-lg p-2">
                        <template x-for="coach in allCoaches" :key="coach.id">
                            <label class="flex items-center gap-2 p-2 rounded-lg border border-slate-200">
                                <input type="checkbox" :value="coach.id" x-model="createForm.coach_ids" class="rounded border-slate-300 text-[#1a307b]">
                                <span class="text-sm text-slate-700" x-text="coach.name"></span>
                            </label>
                        </template>
                    </div>
                ...
                <div class="flex justify-end gap-2">
                    <button @click="createModal=false" class="px-4 py-2 rounded-lg border border-slate-300 text-sm">Batal</button>
                    <button @click="createSlot()" :disabled="saving" class="px-4 py-2 rounded-lg bg-[#1a307b] text-white text-sm font-semibold disabled:opacity-50">
                        <span x-show="!saving">Simpan</span>
                        <span x-show="saving">Menyimpan...</span>
                    </button>
                </div>
    ...
    return {
        ...
        async loadSessionTimes() {
            try {
                ...
                this.sessionTimes = [];
                window.showToast(error?.message || 'Gagal memuat master session time.', 'error');
            }
        },
        ...
        async createSlot() {
            ...
            try {
                ...
                window.showToast('Slot berhasil ditambahkan.', 'success');
                await this.loadSessionDetail();
            } catch (error) {
                window.showToast(error?.message || 'Gagal menambahkan slot.', 'error');
            } finally {
                this.saving = false;
            }
        },
        async deleteSlot(slot) {
            ...
            const confirmed = window.confirm(`Hapus slot ${slot.time_name}?`);
            if (!confirmed) return;

            this.saving = true;
            try {
                const response = await window.API.delete(`/admin/training-session-slots/${slot.id}`);
                window.showToast(response?.message || 'Slot berhasil dihapus.', 'success');
                await this.loadSessionDetail();
            } catch (error) {
                window.showToast(error?.message || 'Gagal menghapus slot.', 'error');
            } finally {
                this.saving = false;
            }
        },
