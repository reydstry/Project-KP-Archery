...
<div class="min-h-screen bg-white px-2 py-2 sm:p-8">
    ...
    <div id="sessionDetailsCard" class="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-slate-200/60 p-2 sm:p-4 mb-2 sm:mb-4 hidden card-animate" style="animation-delay: 0.2s">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            ...
            <a href="{{ route('coach.sessions.index') }}" class="sm:ml-auto px-3 py-1.5 sm:px-4 sm:py-2 bg-white hover:bg-slate-50 text-slate-700 rounded-lg font-medium border border-slate-200 transition-all duration-200 text-xs flex items-center justify-center gap-1.5">
                Lihat Sesi
            </a>
        </div>
    ...
    <div id="markAbsentModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <!-- Background overlay -->
            <div class="fixed inset-0 transition-opacity bg-gray-900 bg-opacity-75" onclick="closeMarkAbsentModal()"></div>
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="bg-white px-6 pt-6 pb-4">
                    ...
                    <div class="mt-5 text-center">
                        <h3 class="text-2xl font-bold text-gray-900 mb-2">Mark All as Absent?</h3>
                        <p class="text-gray-600 mb-1">This will mark all participants as absent.</p>
                        <p class="text-sm text-gray-500 mt-4">You can change individual statuses later if needed.</p>
                    </div>
                </div>
                <div class="bg-gray-50 px-6 py-4 flex flex-col-reverse sm:flex-row gap-3 justify-center">
                    <button type="button" onclick="closeMarkAbsentModal()" class="w-full sm:w-auto px-6 py-3 bg-white hover:bg-gray-100 text-gray-700 rounded-xl font-medium border border-gray-300 transition-all duration-200">
                        Cancel
                    </button>
                    <button type="button" onclick="confirmMarkAllAbsent()" class="w-full sm:w-auto px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-medium transition-all duration-200 shadow-lg">
                        Mark All Absent
                    </button>
                </div>
            </div>
...
function loadAttendanceData(sessionId, slotId) {
    ...
    showNotification('Kehadiran kini dikelola dari input admin per sesi.', 'info');
}
...
function renderParticipants(filteredParticipants = participants) {
    ...
    if (filteredParticipants.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="3" class="px-2 sm:px-3 py-8 text-center text-xs sm:text-sm text-slate-600">No participants found</td>
            </tr>
        `;
        return;
    }
...
function confirmMarkAllAbsent() {
    ...
    showNotification('All participants marked as absent', 'success');
}

function saveAttendance() {
    if (!currentSessionId) return;

    showNotification('Validasi attendance manual di halaman ini sudah dinonaktifkan.', 'info');
    return;
    ...
    const tasks = participants
        .map(p => {
            ...
            const url = `/coach/attendance/${p.id}`;
