...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 p-8">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4 mb-4">
            ...
            <div>
                <h1 class="text-4xl font-bold text-slate-900">Edit Session</h1>
                <p class="text-slate-600 text-lg mt-1">Update training session details</p>
            </div>
        </div>
    </div>
    ...
    <div id="loadingState" class="text-center py-12">
        ...
        <p class="text-slate-600 mt-4">Loading session data...</p>
    </div>

    <!-- Form (Hidden initially) -->
    <form id="editSessionForm" class="max-w-4xl hidden">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-slate-200/60 p-8 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="current_participants" readonly
                        class="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-slate-600 cursor-not-allowed">
                </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="allow_bookings" name="allow_bookings" class="sr-only peer">
                ...
                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                    <div>
                        ...
                        <p class="text-sm text-slate-600 mt-1">Notify participants about updates</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" id="send_notifications" name="send_notifications" class="sr-only peer">
                        <div class="w-14 h-7 bg-slate-300 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[4px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-blue-600"></div>
                    </label>
                </div>
...
function loadSessionData() {
    fetch(`/coach/sessions/${sessionId}/edit`, {
        headers: {
    ...
    .then(data => {
        if (data.session) {
            ...
            document.getElementById('editSessionForm').classList.remove('hidden');
        }
    })
    .catch(error => {
        ...
        document.getElementById('loadingState').innerHTML = `
            <div class="text-center py-12">
                ...
                <p class="text-slate-600 font-medium">Failed to load session data</p>
                <a href="{{ route('coach.sessions.index') }}" class="text-blue-600 hover:text-blue-700 mt-2 inline-block">Back to Sessions</a>
            </div>
        `;
    });
}
...
function populateForm(session) {
    ...
    document.getElementById('status').value = session.status || 'upcoming';
    ...
    document.getElementById('max_participants').value = session.maxParticipants || '';
    document.getElementById('current_participants').value = session.participants || '0';
    document.getElementById('equipment').value = session.equipment || '';
    document.getElementById('allow_bookings').checked = session.allowBookings || false;
    document.getElementById('send_notifications').checked = session.sendNotifications || false;
}
...
document.getElementById('editSessionForm').addEventListener('submit', function(e) {
    ...
    fetch(`/coach/sessions/${sessionId}`, {
        method: 'PUT',
        headers: {
