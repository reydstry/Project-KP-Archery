...
<div x-data="dashboardData()" x-init="fetchDashboard()">
    ...
    <div x-show="!loading" x-cloak class="space-y-6">
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="px-4 py-2 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20">
                            <p class="text-slate-300 text-xs mb-0.5">Status Member</p>
                            <p class="text-white font-bold text-sm capitalize flex items-center gap-2">
        ...
        <div x-show="data.quota" class="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-500 to-blue-700 rounded-3xl shadow-2xl">
            ...
            <div class="relative p-8">
                <div class="flex items-start justify-between mb-6">
                    <div>
                        <div class="inline-flex items-center gap-2 px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full mb-3">
                            ...
                            <span class="text-white text-sm font-semibold">Paket Aktif</span>
                        </div>
                    ...
                    <div class="relative w-24 h-24">
                        <svg class="transform -rotate-90 w-24 h-24">
                            <circle cx="48" cy="48" r="40" stroke="currentColor" stroke-width="8" fill="transparent" class="text-white/20"/>
                        ...
                        <div class="absolute inset-0 flex items-center justify-center">
                            <div class="text-center">
                                <p class="text-2xl font-bold text-white" x-text="Math.round((data.quota?.used_sessions / data.quota?.total_sessions) * 100)"></p>
                                <p class="text-white/80 text-xs">%</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4 mb-6">
                    <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-4">
                        <p class="text-blue-100 text-xs mb-1">Total Sesi</p>
                        <p class="text-3xl font-bold text-white" x-text="data.quota?.total_sessions"></p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-4">
                        <p class="text-blue-100 text-xs mb-1">Terpakai</p>
                        <p class="text-3xl font-bold text-white" x-text="data.quota?.used_sessions"></p>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-4">
                        <p class="text-green-200 text-xs mb-1">Tersisa</p>
                        <p class="text-3xl font-bold text-white" x-text="data.quota?.remaining_sessions"></p>
                    </div>
                </div>

                <div class="flex items-center justify-between gap-4">
                    <div class="flex-1 bg-white/10 backdrop-blur-sm rounded-2xl p-4">
                        <p class="text-blue-100 text-xs mb-1">Sisa Waktu Paket</p>
                    ...
                    <a href="{{ route('member.bookings.create') }}"
                       class="group px-8 py-4 bg-white text-blue-600 rounded-2xl font-bold hover:shadow-2xl hover:scale-105 transition-all duration-300 inline-flex items-center gap-3">
        ...
        <!-- No Active Package -->
        <div x-show="!data.quota" class="relative overflow-hidden bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-200 rounded-3xl shadow-lg">
            <div class="absolute top-0 right-0 opacity-10">
                <svg class="w-64 h-64" viewBox="0 0 200 200" fill="currentColor" class="text-amber-900">
                    <path d="M100 0L100 200M0 100L200 100" stroke="currentColor" stroke-width="1" fill="none"/>
                </svg>
            </div>
            <div class="relative p-8 flex items-start gap-6">
                ...
                <div class="flex-1">
                    <h3 class="text-2xl font-bold text-amber-900 mb-2">Paket Belum Aktif</h3>
                    ...
                    <a href="{{ route('member.membership') }}"
                       class="inline-flex items-center gap-2 px-6 py-3 bg-amber-600 text-white rounded-xl font-semibold hover:bg-amber-700 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <!-- Total Hadir -->
            <div class="group relative overflow-hidden bg-gradient-to-br from-emerald-500 to-green-600 rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105">
                ...
                <div class="relative p-6">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <div class="text-right">
                            <p class="text-emerald-100 text-sm mb-1">Tingkat Kehadiran</p>
                    ...
                    <div class="border-t border-white/20 pt-4">
                        <p class="text-white text-4xl font-bold mb-1" x-text="data.attendance?.statistics?.total_attended || 0"></p>
                        <p class="text-emerald-100 text-sm">Total Sesi Hadir</p>
                    </div>
                </div>
            </div>

            <!-- Total Tidak Hadir -->
            <div class="group relative overflow-hidden bg-gradient-to-br from-rose-500 to-pink-600 rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105">
                <div class="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16"></div>
                <div class="relative p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div class="w-14 h-14 bg-white/20 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
                        ...
                        <div class="text-right">
                            <p class="text-rose-100 text-sm mb-1">Perlu Diperbaiki</p>
                            <p class="text-white text-2xl font-bold">
                    ...
                    <div class="border-t border-white/20 pt-4">
                        <p class="text-white text-4xl font-bold mb-1" x-text="data.attendance?.statistics?.total_absent || 0"></p>
                        <p class="text-rose-100 text-sm">Total Sesi Tidak Hadir</p>
                    </div>
                </div>
            </div>

            <!-- Total Prestasi -->
            <div class="group relative overflow-hidden bg-gradient-to-br from-amber-500 to-orange-600 rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105">
                <div class="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16"></div>
                <div class="relative p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div class="w-14 h-14 bg-white/20 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
                        ...
                        <div class="text-right">
                            <p class="text-amber-100 text-sm mb-1">Achievement</p>
                            <p class="text-white text-2xl font-bold">Terbaru</p>
                        </div>
                    </div>
                    <div class="border-t border-white/20 pt-4">
                        <p class="text-white text-4xl font-bold mb-1" x-text="data.achievements?.length || 0"></p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div class="lg:col-span-3 bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('member.bookings') }}" class="text-blue-600 hover:text-blue-800 font-semibold text-sm inline-flex items-center gap-1 hover:gap-2 transition-all">
                ...
                <div class="p-6">
                    ...
                    <div x-show="!data.attendance?.history || data.attendance?.history?.length === 0"
                        ...
                        <p class="text-slate-500 text-sm mb-6">Booking sesi latihan pertama Anda untuk memulai</p>
                        <a href="{{ route('member.bookings.create') }}"
            ...
            <div class="lg:col-span-2 bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-200 bg-gradient-to-r from-amber-50 to-white">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                ...
                <div class="p-6">
                    <div x-show="data.achievements?.length > 0" class="space-y-4">
                        <template x-for="(achievement, index) in data.achievements?.slice(0, 4)" :key="achievement.id">
                            <div class="group relative p-5 rounded-2xl border-2 border-amber-100 hover:border-amber-300 hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-white to-amber-50/30"
                                 :style="`animation: slideIn 0.3s ease-out ${index * 0.1}s backwards`">
                                <div class="flex items-start gap-4">
                                    <div class="flex-shrink-0 w-14 h-14 bg-gradient-to-br from-amber-400 via-amber-500 to-orange-500 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                                        <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872m5.007 0H9.497m5.007 0a7.454 7.454 0 01-.982-3.172M9.497 14.25a7.454 7.454 0 00.981-3.172M5.25 4.236c-.982.143-1.954.317-2.916.52A6.003 6.003 0 007.73 9.728M5.25 4.236V4.5c0 2.108.966 3.99 2.48 5.228M5.25 4.236V2.721C7.456 2.41 9.71 2.25 12 2.25c2.291 0 4.545.16 6.75.47v1.516M7.73 9.728a6.726 6.726 0 002.748 1.35m8.272-6.842V4.5c0 2.108-.966 3.99-2.48 5.228m2.48-5.492a46.32 46.32 0 012.916.52 6.003 6.003 0 01-5.395 4.972m0 0a6.726 6.726 0 01-2.749 1.35m0 0a6.772 6.772 0 01-3.044 0"/>
                                        </svg>
                                    </div>
                        ...
                        <a href="{{ route('member.achievements') }}"
                           class="block w-full text-center py-3 px-4 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-2xl font-semibold hover:shadow-lg hover:scale-105 transition-all duration-300">
